 ...
 * This is a reusable helper used by both single and bulk form filling
 */
export async function fillForm2Fields(
  form: PDFForm,
  enrollment: NonNullable<Enrollment>,
  branchConfig: Awaited<ReturnType<typeof getBranchConfigWithTenant>>
) {
  ...
  form.getTextField(form2FieldNames.current_village).setText(client.addressLine2.toUpperCase());
  if (client.addressLine3)
    form.getTextField(form2FieldNames.current_state).setText(client.addressLine3.toUpperCase());
  ...
  if (learningLicense && learningLicense.licenseNumber) {
    ...
    if (learningLicense.issueDate) {
      form.getTextField(form2FieldNames.validity_period_from).setText(learningLicense.issueDate);
    }
  ...
  form
    .getTextField(form2FieldNames.declaration_guardian_name_2)
    .setText(guardianFullName.toUpperCase());

  form
    .getTextField(form2FieldNames.declaration_guardian_relationship_2)
    .setText(client.guardianRelationship.toUpperCase());
}
...
export const fillForm2 = async (clientId: string) => {
  try {
    ...
    return {
      success: true,
      pdfData: base64Pdf,
      fileName: `${enrollment.client.clientCode}-form-2.pdf`,
    };
  } catch (error) {
